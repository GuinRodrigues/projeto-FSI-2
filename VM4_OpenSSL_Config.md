...
# 8. Certificado do OCSP Responder (com OCSPSigning EKU — não usa a chave da CA directamente)
sudo openssl genrsa -out /etc/pki/CA/ocsp.key 2048
sudo openssl req -new -key /etc/pki/CA/ocsp.key -out /etc/pki/CA/ocsp.csr
sudo bash -c 'printf "keyUsage = critical, digitalSignature\nextendedKeyUsage = critical, OCSPSigning\nbasicConstraints = CA:FALSE\nsubjectKeyIdentifier = hash\nauthorityKeyIdentifier = keyid,issuer\n" > /etc/pki/CA/v3_ocsp.ext'
sudo bash -c 'cd /etc/pki/CA && openssl ca -in ocsp.csr -cert ca.crt -keyfile ca.key -out ocsp.crt -extfile v3_ocsp.ext -policy policy_anything -batch'

# 9. Iniciar OCSP Responder
sudo bash -c 'cd /etc/pki/CA && openssl ocsp -index index.txt -port 8080 -rsigner ocsp.crt -rkey ocsp.key -CA ca.crt -text'